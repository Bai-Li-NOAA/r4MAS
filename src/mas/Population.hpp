...
#warning needs to be more efficient, possibly allocating unused memory
        ...
        inline REAL_T CalculateUnfishedSpawningBiomassPerRecruit() {
            ...
            for (int y = 0; y < this->years; y++) {
                ...
                variable ntemp0 = 1.0;
                for (int s = 0; s < this->seasons; s++) {
                    std::vector<variable> unfished_surviability(ages.size());



                    for (int a = 0; a < ages.size() - 1; a++) {
                        if (a == 0) {
                            unfished_surviability[a] = 1.0;
                        } else {
                            //                            unfished_surviability[a] = mas::exp(-1.0*(this->M[a])+this->)
                        }
                        size_t index = y * this->seasons * this->ages.size()
                                + (s) * this->ages.size() + a;
        ...
        void CalculateInitialNumbersEquilibrium() {
            ...
            variable init_f = this->initialF; //0.0;//???whats a good number to use
            ...
            for (a = 1; a < this->ages.size(); a++) {
                this->initial_equilibrium_numbers[a] =
                        ...
                        * mas::exp(static_cast<REAL_T> (-1.0)* (this->M[a - 1]) - init_f);
            }
            ...
            for (a = this->ages.size(); a < this->initial_equilibrium_numbers.size(); a++) {
                ...
                this->initial_equilibrium_numbers[a] =
                        this->initial_equilibrium_numbers[a - 1]
                        * mas::exp(static_cast<REAL_T> (-1.0) * (m) - init_f);
            ...
            this->initial_equilibrium_numbers[this->ages.size() - 1] +=
                    ...
                    - 1] * mas::exp(static_cast<REAL_T> (-1.0) * (m)) - init_f) / (1.0 - mas::exp(static_cast<REAL_T> (-1.0) * (m)) - init_f);
        }
...
#warning switch to observed catch biomass
        ...
        inline void CalculateMortality(const int &year, const int &season,
            ...
            if (age == 0) {
                ...
                S[index] = mas::exp(-1.0 * Z[index]) * S[index - 1];
            }
            sum_of_Z += Z[index];
        ...
        void CalculateMSY2(REAL_T maxF = 1.0, REAL_T step = 0.001) {
            /**
